...
file = open('monk-abilities.html', 'r')
file = file.read()
file = file.replace('\n', '')
...
def print_abilities_list():
    for group in data:
        ...
        print_property('effects', re.sub(r'/revision.*?"', '"', group[5].replace("'", "\\'")))
        print_property('speed', group[6])
        print_property('interrupt', group[7])
        print_property('duration', group[8])
        print_property('description', group[9])
        print('},')
