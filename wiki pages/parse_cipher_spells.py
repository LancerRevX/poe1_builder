...
file = open('cipher-spells.html', 'r')
file = file.read()
file = file.replace('\n', '')
...
def print_spells():
    for level in range(8):
        ...
        for spell in spells:
            ...
            print_property('focus', spell[3])
            print_property('effects', spell[4].replace("'", "\\'"))
            print_property('speed', spell[5])
            print_property('duration', spell[6])
            print_property('range', spell[7])
            print_property('area', spell[8])
